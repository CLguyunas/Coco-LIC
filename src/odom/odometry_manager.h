...
namespace cocolic
{
  ...
  protected:
    ...
    void BuildVisualObs(int64_t image_timestamp);
    void PrepareQiVisualObs(int64_t image_timestamp);
    void PrepareQiLidarObs(const Eigen::aligned_vector<PointCorrespondence> &point_corrs);
    ...
    double ComputeLidarResidual(const PointCorrespondence &corr, const SE3d &T_lidar) const;
    double ComputeVisualResidual(const QiVisualObs &obs, const SE3d &T_cam) const;
    Eigen::Vector3d TransformLidarPoint(const PointCorrespondence &corr, const SE3d &T_lidar) const;
    ...
    double QiConditionNumber(const Eigen::Matrix<double, 6, 6> &mat) const;
